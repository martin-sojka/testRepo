...
public class SysTray {
	...
	public void displayTray() throws AWTException, MalformedURLException {
    	...
    	//	System.out.println(String.format(jira_user.getEmailAddress()));
    		...
    		Promise<SearchResult> search = rc.getSearchClient().searchJql("project = IIS AND issuetype in (Bug, \"IT Help\", \"Service Request\", \"Service Request with Approvals\", Task, Sub-task) AND status in (\"In Progress\", Open, \"Waiting for 3rd Party\", \"Waiting for Approval\", \"Waiting for Client\") AND updated >= -5m AND assignee in (L2, Lubomir.zalesak, martin.sojka, Tomas.lachout) order by created DESC");
